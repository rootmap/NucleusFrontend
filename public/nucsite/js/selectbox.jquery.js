/* NOTE! RENAMED TO SELECTBOX TO PREVENT CONFLICT W OLD EASYSELECTBOX
 *
 *
		...
		init : function( options ) {
			...
			return this.each(function() {
				...
				if('select'!=selectObj[0].nodeName.toLowerCase()){//check if its a <select> tag
					...
					easySelectHtml = '<div tabindex="0" class="select-box '+className+'">'+displayerHtml+dropdownHtml+'</div>';
					
					//add to dom
					easySelect = $(easySelectHtml).insertAfter(selectObj);
					easySelect.addClass('easy-select-'+$('body').find('.'+options.classNames.selectbox).index(easySelect));//add a class based on selectbox count
