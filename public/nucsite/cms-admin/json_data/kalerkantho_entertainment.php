...
//step1
$result = file_get_contents('http://www.kalerkantho.com/online/entertainment', false, $context);
...
$detailPatternlink = '/<p class=\"hidden-xs hidden-sm\">(.*?)<\/p>/si';
...
$longdetailPatternlink = '{<div\s+class="some-class-name2"\s*>((?:(?:(?!<div[^>]*>|</div>).)++|<div[^>]*>(?1)</div>)*)</div>}si';
...
if ($obj->exists_multiple("compose_news", $exists_array) == 0) {
    ...
    $extension=$img['1'];
    $newname_image="kalerkantho_entertainment_".time().".".$extension;
