...
//step1
$result = file_get_contents('http://www.dailynayadiganta.com/news/83/0/%E0%A6%B0%E0%A6%BE%E0%A6%9C%E0%A6%A8%E0%A7%80%E0%A6%A4%E0%A6%BF');
//echo $result;
//bot 1st step complete
//bot grab text start
//$sPattern="/<div class=\"homeTopLeadSlider\">(.*?)<\/div>/s";
//$sPattern = "/<div class=\"topNewsLeftTop\">(.*?)<\/div>/s";
$sPattern='{<div\s+class="topNewsLeftTop"\s*>((?:(?:(?!<div[^>]*>|</div>).)++|<div[^>]*>(?1)</div>)*)</div>}si';
$sText = $result;
...
if ($obj->exists_multiple("compose_news", $exists_array) == 0) {
    ...
    $extension=$img['1'];
    $newname_image="dailynayadiganta_politics_".time().".".$extension;
