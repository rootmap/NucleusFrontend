...
//step1
$result = file_get_contents('http://www.kalerkantho.com/', false, $context);
...
$sPattern='{<div\s+class="col-xs-6"\s*>((?:(?:(?!<div[^>]*>|</div>).)++|<div[^>]*>(?1)</div>)*)</div>}si';
$sText = $result;
...
//step2
$headingPattern =  "/<h2>(.*?)<\/h2>/s";
$headingtext = $aMatch[0][0];
preg_match_all($headingPattern, $headingtext, $headingMatch);

$news_heading_rep = str_replace('<h2>',"",$headingMatch[0][0]);
$news_heading = str_replace('</h2>',"",$news_heading_rep);
...
$longdetail_content = str_replace('   ', "", strip_tags($longdetaillinkMatch[0][0]));
...
//insert system
$exists_array = array("news_headding"=>$news_heading,"reporter"=>4,"news_thumble"=>$img_link);
if ($obj->exists_multiple("compose_news", $exists_array) == 0) {
    ...
    $extension=$img['1'];
    $newname_image="kalerkantho_".time().".".$extension;
