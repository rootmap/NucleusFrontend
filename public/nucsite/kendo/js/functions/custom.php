...
$(document).ready(function() {
	...
	$('#calendar').fullCalendar({
		...
		editable: true,
		events: <?php //include('../class/db_Class.php'); $obj=new db_class(); ?>
[
  <?php 
  if($input_status==1)
  {
	$sqlevent=$obj->SelectAll("calender_event");  
  }
  else
  {
	$sqlevent=$obj->SelectAllByID("calender_event",array("store_id"=>$input_by));  
  }
  
  $total=count($sqlevent);
  $a=1;
  if(!empty($sqlevent))
