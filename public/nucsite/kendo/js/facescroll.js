	...
	var methods = {
		init : function(options) {
			this.each(function() {
				...
				if (parseInt($horizontalBar.css('height')) == 0 || $horizontalBar.css('height') == 'auto') $horizontalBar.css({ 'height': 1, 'border-radius': 3, 'cursor': 'default' });
...
//for scrill any data create or relace with demo 1
jQuery(function(){ // on page DOM load
	$('#demo1').alternateScroll({ 'vertical-bar-class': 'styled-v-bar', 'hide-bars': false });	
	$('#lastnewsscroll').alternateScroll({ 'vertical-bar-class': 'styled-v-bar', 'hide-bars': false });
        $('#lastnewsscrolls').alternateScroll({ 'vertical-bar-class': 'styled-v-bar', 'hide-bars': false });
})
