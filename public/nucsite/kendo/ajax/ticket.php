...
elseif($st==6915)
{
	...
	$chk=$obj->SelectAllByVal($table,$field,$ticket_id,$status);
	?>
    <select name="<?php echo $auto_id; ?>" onchange="TicketWorkChanged('<?php echo $table; ?>','<?php echo $field; ?>','<?php echo $ticket_id; ?>','<?php echo $status; ?>','<?php echo $auto_id; ?>','<?php echo $fetchplace; ?>')" id="<?php echo $auto_id; ?>">
    	<option <?php echo $obj->selected($chk,1); ?> value="1">Yes</option>
        <option <?php echo $obj->selected($chk,0); ?> value="0">No</option>
    </select>
    <?php
...
elseif($st==888)
{
	...
	$chk=$obj->SelectAllByVal($table,$field,$ticket_id,$status);
	?>
    <select name="<?php echo $auto_id; ?>" onchange="PaymentMethodChange('<?php echo $table; ?>','<?php echo $field; ?>','<?php echo $ticket_id; ?>','<?php echo $status; ?>','<?php echo $auto_id; ?>','<?php echo $fetchplace; ?>')" id="<?php echo $auto_id; ?>">
    	<?php 
		$sqlmethod=$obj->SelectAll("payment_method");
		if(!empty($sqlmethod))
		foreach($sqlmethod as $method):
		?>
    	<option <?php echo $obj->selected($chk,$method->id); ?> value="<?php echo $method->id; ?>"><?php echo $method->meth_name; ?></option>
        <?php 
		endforeach;
		?>
    </select>
...
elseif($st==2034)
{
	...
	$chk=$obj->SelectAllByVal($table,$field,$ticket_id,$status);
	?>
    <select name="<?php echo $auto_id; ?>" onchange="WarrentyChanged('<?php echo $table; ?>','<?php echo $field; ?>','<?php echo $ticket_id; ?>','<?php echo $status; ?>','<?php echo $auto_id; ?>','<?php echo $fetchplace; ?>','<?php echo $pid; ?>')" id="<?php echo $auto_id; ?>">
    	<?php 
		$warrenty=$obj->SelectAllByVal("product","id",$pid,"warranty");
		for($i=0; $i<=$warrenty; $i++):
		?>
...
{
	?>
	<span onclick="PaymentMethod('<?php echo $table; ?>','<?php echo $field; ?>','<?php echo $val; ?>','<?php echo $uval; ?>','<?php echo $fetchplace; ?>')">
	<?php
	if($uval==1){ $ff=$obj->SelectAllByVal("payment_method","id",$uval,"meth_name"); }else{ $ff=$obj->SelectAllByVal("payment_method","id",$uval,"meth_name"); }
	if($obj->update($table,array($field=>$val,$ufield=>$uval))==1)
	{
		echo "<label>".$ff."</label>";
	}
...
elseif($st==2371)
{
	...
	if($obj->update($table,array($field=>$val,$ufield=>$uval))==1)
	{
		echo "<label>".$uval." Days remaining</label>";
	}
	else
	{
		echo "<label>".$uval." Days remaining</label>";
