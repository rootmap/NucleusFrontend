...
if ($cashier_id == 1) {
    ...
    if (empty($_POST['pam']) || $_POST['pam'] == 0) {
        $obj->Error("Your Payment is Empty / Zero Amount Please Check Your Paid Amount.", "pos_redirect.php");
    } else {
        ...
        if ($_POST['customername'] == 6) {
            ...
            if ($obj->insert($table4, array("invoice_id" => $_POST['sidd'], "payment_type" => $_POST['customername'], "cid" => $_POST['cid'], "invoice_creator" => $input_by, "input_by" => $input_by, "access_id" => $access_id, "amount" => $_POST['pam'], "date" => date('Y-m-d'), "datetime" => date('Y-m-d H:i:s'), "time" => time(), "status" => 1)) == 1) {

                $obj->insert($table4, array("invoice_id" => $_POST['sidd'], "payment_type" => $_POST['customername'], "cid" => $_POST['cid'], "invoice_creator" => $input_by, "input_by" => $input_by, "access_id" => $access_id, "amount" => $_POST['pamc'], "date" => date('Y-m-d'), "datetime" => date('Y-m-d H:i:s'), "time" => time(), "status" => 1));

                $obj->update("sales", array("sales_id" => $_POST['sidd'], "payment_method" => $_POST['customername'], "datetime" => date('Y-m-d H:i:s'), "input_by" => $input_by));
