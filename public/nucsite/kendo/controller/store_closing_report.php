...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'csd.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`csd`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `csd`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT csd.* FROM close_store_detail AS csd
                    " . $migratecond . " 
                    ORDER BY csd.id DESC LIMIT $start,$pagesize";
        ...
        $sqlpars = "SELECT csd.* FROM close_store_detail AS csd
        ...
        include '../plugin/plugin.php';
        $plugin = new CmsRootPlugin();
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'csd.', ' WHERE ');
        ...
        if ($input_status == 1) {
            ...
            $newcond = " `csd`.`store_id`='" . $input_by . "'";
            $migratecond = $cond . $newcond;
        }
        
        $sql_coustomer = $obj->FlyQuery("SELECT csd.* FROM close_store_detail AS csd 
                                        " . $migratecond . " 
                                        ORDER BY csd.id DESC LIMIT $start,$pagesize");



        $totalrows = $obj->FlyQuery("SELECT csd.* FROM close_store_detail AS csd
                                    " . $migratecond . " 
                                    ORDER BY csd.id DESC", 1);
