...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'tcf.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`tcf`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `tcf`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT  
                    ...
                    tcf.date
                    FROM
                    ticket_custom_field AS tcf
                    " . $migratecond . " 
                    ORDER BY tcf.id DESC LIMIT $start,$pagesize";
