...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'bdt.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`bdt`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `bdt`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    ...
                    bdt.name,
                    bdt.date
                    FROM buyback_device_type AS bdt
                    ORDER BY bdt.id DESC LIMIT $start,$pagesize";
