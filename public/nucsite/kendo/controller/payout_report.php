...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`p`.`uid`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `p`.`uid`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    p.id,
                    (SELECT `transaction` FROM transaction_log WHERE track_id=p.track_id LIMIT 1) AS track_id,
                    (SELECT name from store where store.store_id=p.uid LIMIT 1) AS store_name,
                    (SELECT name from store where store.id=p.cashier_id LIMIT 1) as cashier_name,
