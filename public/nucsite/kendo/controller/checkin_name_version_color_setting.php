...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'cvc.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`cvc`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `cvc`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    cvc.id,
                    cvc.checkin_version_id,
                    cvc.name AS checkin_name,
                    cv.name AS checkin_version,
                    ci.name AS checkin,
                    cvc.date
                    FROM checkin_version_color AS cvc
                    LEFT JOIN checkin_version AS cv ON cvc.checkin_version_id=cv.id
                    LEFT JOIN checkin AS ci ON cvc.checkin_id=ci.id 
                    " . $migratecond . " 
                    ORDER BY cvc.id DESC LIMIT $start,$pagesize";
