...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'plc.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`plc`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `plc`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    ...
                    plc.name,
                    plc.code
                    FROM product_lcd_color AS plc
                    ORDER BY plc.id DESC LIMIT $start,$pagesize";
