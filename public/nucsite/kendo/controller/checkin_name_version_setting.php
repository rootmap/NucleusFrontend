...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'cv.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`cv`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `cv`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    cv.id as checkin_version_id,
                    cv.name as checkin_name,
                    ci.name as checkin,
                    cv.date
                    FROM checkin_version AS cv
                    LEFT JOIN checkin as ci ON cv.checkin_id=ci.id 
                    " . $migratecond . " 
                    ORDER BY cv.id DESC LIMIT $start,$pagesize";
