...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'tv.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`tv`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `tv`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    ...
                    tv.title,
                    tv.name,
                    CASE tv.status WHEN 1 THEN 'Publish'
                    ELSE CASE tv.status WHEN 2 THEN 'Unpublish'
                    ELSE 'Undefined'
                    END END AS `status`

                    FROM tutorial_video AS tv
                    WHERE tv.status ='1'
                    ORDER BY tv.id DESC LIMIT $start,$pagesize";
