...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'sc.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`sc`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `sc`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT sc.*,
                    ...
                    ORDER BY sc.id DESC LIMIT $start,$pagesize";
        ...
        if ($input_status == 1) {
            ...
            $newcond = " `sc`.`store_id`='" . $input_by . "'";
            $migratecond = $cond . $newcond;
        }

        $sql_coustomer = $obj->FlyQuery("SELECT sc.*,
                                        c.name AS currency,
                                        c.sign AS currency_sign
                                        FROM store_currency AS sc
                                        LEFT JOIN currency AS c ON c.id=sc.currency_id
        ...
        $totalrows = $obj->FlyQuery("SELECT sc.*,
                                    c.name AS currency,
                                    c.sign AS currency_sign
                                    FROM store_currency AS sc
                                    LEFT JOIN currency AS c ON c.id=sc.currency_id
                                    WHERE sc.store_id='$store'
                                    ORDER BY sc.id DESC", 1);
