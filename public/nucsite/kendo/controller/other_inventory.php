...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        /*
         * where ((`pos_multi`.`product`.`description` <> 'Product Added From Ticket') and 
         * (`pos_multi`.`product`.`description` <> 'Product Added From Unlock') and 
         * (`pos_multi`.`product`.`description` <> 'Product Added From Checkin') and 
         * (`pos_multi`.`product`.`status` <> '4') and 
         * (`pos_multi`.`product`.`status` <> '5')) 
         * order by `pos_multi`.`product`.`name`
         */
          ...
          $sqlpar = "select 
                    ...
                    where `p`.`description` <> 'Product Added From Ticket'  
                    AND `p`.`description` <> 'Product Added From Unlock' 
                    AND `p`.`description` <> 'Product Added From Checkin' 
                    AND `p`.`status` <> '4' 
                    AND `p`.`status` <> '5' 
                    AND `p`.`status` <> '6' 
                    " . $migratecond . " 
