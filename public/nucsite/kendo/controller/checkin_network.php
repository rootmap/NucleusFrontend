...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'cn.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`cn`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `cn`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    cn.id AS checkin_network_id,
                    cn.name AS network_name,
                    ci.name AS checkin,
                    cn.date
                    FROM checkin_network AS cn
                    LEFT JOIN checkin AS ci ON cn.checkin_id=ci.id
                    " . $migratecond . " 
                    ORDER BY cn.id DESC LIMIT $start,$pagesize";
