...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'ip.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`ip`.`input_by`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `ip`.`input_by`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    ip.id,
                    ip.invoice_id AS sales_id,
                    pm.meth_name AS tender_name,
                    CONCAT(c.firstname,'',c.lastname) AS customer,
                    ip.amount,
                    ip.date
                    FROM invoice_payment AS ip
                    LEFT JOIN payment_method AS pm ON pm.id=ip.payment_type
                    LEFT JOIN coustomer AS c ON c.id=ip.cid

                    " . $migratecond . " 
                    ORDER BY ip.id DESC LIMIT $start,$pagesize";
