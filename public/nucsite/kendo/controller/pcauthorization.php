...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'sup.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`sup`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `sup`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    ...
                    sup.pc_address,
                    sup.date
                    FROM setting_user_pc AS sup
                    ORDER BY sup.id DESC LIMIT $start,$pagesize";
