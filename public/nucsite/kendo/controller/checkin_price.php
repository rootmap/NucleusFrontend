...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'cpr.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`cpr`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `cpr`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    cpr.id,
                    cpr.barcode,
                    ci.name AS checkin_device,
                    cv.name AS checkin_version,
                    cp.name AS checkin_problem,
                    CONCAT('$',cpr.name) AS checkin_price,
                    cpr.date
                    FROM checkin_price AS cpr
                    LEFT JOIN checkin AS ci ON cpr.checkin_id=ci.id
                    LEFT JOIN checkin_version AS cv ON cpr.checkin_version_id=cv.id
                    LEFT JOIN checkin_problem AS cp ON cpr.checkin_problem_id=cp.id
                    " . $migratecond . " 
                    ORDER BY cpr.id DESC LIMIT $start,$pagesize";
