...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'cp.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`cp`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `cp`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    cp.id AS checkin_problem_id,
                    cp.name AS problem_name,
                    ci.name AS checkin,
                    cp.date
                    FROM checkin_problem AS cp
                    LEFT JOIN checkin AS ci ON cp.checkin_id=ci.id
                    " . $migratecond . " 
