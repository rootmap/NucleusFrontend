...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'ed.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`ed`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `ed`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    ...
                    c.email,
                    ed.date
                    FROM email_deliver AS ed
                    LEFT JOIN coustomer AS c ON c.id=ed.cid
                    ORDER BY ed.id DESC LIMIT $start,$pagesize";
