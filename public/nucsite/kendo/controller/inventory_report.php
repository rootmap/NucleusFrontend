    ...
    if (isset($_GET['filter'])) {
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                $migratecond = $cond.$operator.$inner_cond;
            } else {
                $confilter = '';
            ...
            $newcond = " `p`.`input_by`='" . $input_by . "'";
            $migratecond = $cond.$operator.$newcond;
        }
       ...
       $sqlpar = "select 
                    ...
                    (SELECT IFNULL(SUM(quantity),0) from sales where sales.pid=p.id) as sold,
                    (IFNULL(p.quantity,0)-(SELECT IFNULL(SUM(quantity),0) from sales where sales.pid=p.id)) as stock,
                    p.date
                    from product as p 
                    " . $migratecond . " 
