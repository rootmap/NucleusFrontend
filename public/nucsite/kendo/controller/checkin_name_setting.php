...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'ci.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`ci`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `ci`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    ...
                    FROM checkin AS ci 
                    " . $migratecond . " 
                    ORDER BY ci.id DESC LIMIT $start,$pagesize";
