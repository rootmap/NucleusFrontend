...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'vp.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`vp`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `vp`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    ...
                    vp.name,
                    vp.status
                    FROM video_player AS vp
                    ORDER BY vp.id DESC LIMIT $start,$pagesize";
