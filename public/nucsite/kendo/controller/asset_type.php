...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'att.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`att`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `att`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                    ...
                    att.name,
                    att.date
                    FROM asset_type AS att
                    " . $migratecond . " 
                    ORDER BY att.id DESC LIMIT $start,$pagesize";
