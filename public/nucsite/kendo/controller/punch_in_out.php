    ...
    if (isset($_GET['filter'])) {
        ...
        if (isset($_GET['cashier_id'])) {
            if (!empty($migratecond)) {
                $migratecond .=" AND spt.cashier_id='".$_GET['cashier_id']."' AND (spt.indate='".date('Y-m-d')."' OR spt.outdate='".date('Y-m-d')."')";
            } else {
                $migratecond .=" WHERE spt.cashier_id='".$_GET['cashier_id']."' AND (spt.indate='".date('Y-m-d')."' OR spt.outdate='".date('Y-m-d')."')";
            }
...
if ($verb == "POST") {
    ...
    $out_time = substr($out_time, 0, strpos($out_time, '('));
    $out_time = date('h:i:s A', strtotime($out_time));

    echo $obj->update("store_punch_time", array("id" => $punchid, "cashier_id" => $cashier_id, "outdate" => $out_date, "outtime" => $out_time));
}
?>
