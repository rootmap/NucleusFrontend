...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'ce.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`ce`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `ce`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT 
                ...
                ce.name,
                ce.detail,
                CONCAT(ce.start_date," - ",ce.start_time) AS startdate,
                CONCAT(ce.end_date," - ",ce.end_time) AS enddate
                FROM calender_event AS ce
                    ORDER BY ce.id DESC LIMIT $start,$pagesize";
