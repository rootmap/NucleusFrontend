...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        if ($input_status == 1) {
         ...
         $totalotherdata = $obj->FlyQuery("SELECT 
                                        COUNT(a.id) as a1,
                                        SUM(a.price) as a2,
                                        (SELECT SUM(price) FROM buyback as a " . $migratecond . " AND a.payment_method='3') as a3,
                                        (SELECT SUM(price) FROM buyback as a " . $migratecond . " AND a.payment_method='4') as a4,
                                        (SELECT SUM(price) FROM buyback as a " . $migratecond . " AND a.payment_method='5') as a5
                                        FROM buyback as a 
                                        " . $migratecond . " 
                                        ORDER BY a.id DESC");
        ...
        if ($total_in_filter >= 1) {
            echo "{\"data\":" . json_encode($filter_result) . ",\"total\":" . $total_in_filter . ",\"otherdata\":" . json_encode($totalotherdata) . "}";
        } else {
        ...
        echo "{\"data\":" . json_encode($sqlCity) . ",\"total\":" . $totalrows . ",\"otherdata\":" . json_encode($totalotherdata) . "}";
    }
}
