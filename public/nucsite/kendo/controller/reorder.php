...
if ($verb == "GET") {
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                    ...
                    foreach ($sqlchain_store_ids as $ch):
                        if ($i == 1) {
                            $inner_cond = "`a`.`store_id`='" . $ch->store_id . "'";
                        } else {
                            $inner_cond = " OR `a`.`store_id`='" . $ch->store_id . "'";
                        }
                        $i++;
                    endforeach;
                    $inner_cond .=')';
            ...
            $migratecond=$cond;
                    ...
                    " . $migratecond . " 
                    order by `p`.`name` ASC LIMIT $start,$pagesize";
  ...
  `sales_id` int(20)
        ...
        if ($input_status == 1) {
            ...
            $newcond = " ".$conds_operate." `a`.`store_id`='" . $input_by . "'";
       ...
       $sql_coustomer = $obj->FlyQuery("SELECT a.id,
                                        a.pid as pid,
                                        p.name,
                                        a.store_id,
                                        p.barcode,
        ...
        $totalrows = $obj->FlyQuery("SELECT a.id,
                                    a.pid as pid,
                                    p.name,
                                    a.store_id,
                                    p.barcode,
