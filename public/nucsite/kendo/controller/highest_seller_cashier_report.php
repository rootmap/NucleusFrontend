...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'alldata.', ' AND ');
...
//        }
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`alldata`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `alldata`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT alldata.*,concat(alldata.total_sales,' Item Sold') as solditem FROM (select 
                                        s.id,
                                        s.store_id,
                                        s.name,
                                        (select SUM(ss.quantity) from sales as ss WHERE ss.cashier_id=s.id GROUP BY ss.cashier_id) as total_sales
                                        from store as s where s.status='3') as alldata 
                                        
                                        " . $migratecond . " 
                                        ORDER BY total_sales DESC LIMIT $start,$pagesize";
        ...
        $sqlpars = "SELECT alldata.*,concat(alldata.total_sales,' Item Sold') as solditem FROM (select 
                                        s.id,
                                        s.store_id,
                                        s.name,
                                        (select SUM(ss.quantity) from sales as ss WHERE ss.cashier_id=s.id GROUP BY ss.cashier_id) as total_sales
        ...
        if ($input_status == 1) {
            ...
            $newcond = " `alldata`.`store_id`='" . $input_by . "'";
        ...
        $sql_coustomer = $obj->FlyQuery("SELECT alldata.*,concat(alldata.total_sales,' Item Sold') as solditem FROM (select 
                                        s.id,
                                        s.store_id,
                                        s.name,
                                        (select SUM(ss.quantity) from sales as ss WHERE ss.cashier_id=s.id GROUP BY ss.cashier_id) as total_sales
        ...
        $totalrows = $obj->FlyQuery("SELECT alldata.*,concat(alldata.total_sales,' Item Sold') as solditem FROM (select 
                                        s.id,
                                        s.store_id,
                                        s.name,
                                        (select SUM(ss.quantity) from sales as ss WHERE ss.cashier_id=s.id GROUP BY ss.cashier_id) as total_sales
