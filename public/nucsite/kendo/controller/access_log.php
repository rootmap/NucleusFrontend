...
if ($verb == "GET") {
    if (isset($_GET['filter'])) {
        ...
        $cond = $plugin->BackEndDateSearch('from', 'to', 'date', 'al.', ' AND ');
        ...
        if ($input_status == 1) {
            ...
            if (!empty($sqlchain_store_ids)) {
                ...
                foreach ($sqlchain_store_ids as $ch):
                    if ($i == 1) {
                        $inner_cond = "`al`.`store_id`='" . $ch->store_id . "'";
                    } else {
                        $inner_cond = " OR `al`.`store_id`='" . $ch->store_id . "'";
                    }
                    $i++;
                endforeach;
                $inner_cond .=')';
        ...
        $sqlpar = "SELECT
                    ...
                    LEFT JOIN store AS s ON s.store_id=al.store_id
                    " . $migratecond . " 
                    ORDER BY al.id DESC LIMIT $start,$pagesize";
