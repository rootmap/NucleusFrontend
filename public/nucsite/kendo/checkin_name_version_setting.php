...
if(@$_GET['del'])
{
	...
	{
		$obj->delete("store_checkin_version",array("sid"=>$_GET['sid']));
...
if(isset($_POST['save']))
{
	...
	if(!empty($name) && !empty($checkin))
	{
		if($input_status==1)
		{
			...
			{
				if($obj->insert($table,array("name"=>$name,"store_id"=>$input_by,"checkin_id"=>$checkin,"date"=>date('Y-m-d'),"status"=>1))==1)
				{
		...
		{
			if($obj->exists_multiple("checkin_version_store",array("store_id"=>$input_by,"name"=>$name,"checkin_id"=>$checkin))==0)
			{
				if($obj->insert($table,array("name"=>$name,"store_id"=>$input_by,"checkin_id"=>$checkin,"date"=>date('Y-m-d'),"status"=>1))==1)
				{
					$new_version_id=$obj->SelectAllByVal3($table,"name",$name,"store_id",$input_by,"checkin_id",$checkin,"id");
					$obj->insert("store_checkin_version",array("id"=>$new_version_id,"store_id"=>$input_by,"date"=>date('Y-m-d'),"status"=>1));
					$obj->Success($name." is Saved Successfully.",$obj->filename());	
				}
				else
				{
...
if(isset($_POST['edit']))
{
	...
	if(!empty($name))
	{
			...
			if($obj->update($table,array("id"=>$id,"name"=>$name,"checkin_id"=>$checkin,"date"=>date('Y-m-d'),"status"=>1))==1)
			{
				$obj->Success($name." is Saved Successfully.",$obj->filename());	
			}
			else
...
<html lang="en">
                ...
                <div class="outer">
                    <div class="inner">
                        <div class="page-header"><!-- Page header -->
                            <h5><i class="font-cogs"></i> Check In Version Setting <?php echo $input_by; ?></h5>
                            <ul class="icons">
    ...
    <input type="hidden" value="<?php echo $_GET['edit']; ?>" name="id">
                                                ...
                                                <div class="control-group">
                                                    ...
                                                    <select name="checkin" style="width: 200px;"  tabindex="2">
                                                        <option value=""></option> 
                                                        <?php
...
<!--                        	<div class="navbar">
                            	<div class="navbar-inner">
                                    <h5><i class="icon-list-alt"></i>CheckIn Version List</h5>
                                        <ul class="icons">
                            ...
                            <div class="table-overflow">
                                ...
                                <div id="grid" style="margin-left: 10px;margin-right: 10px;"></div>
                                        ...
                                        <script id="action_template" type="text/x-kendo-template">
                                            <a class="k-button k-button-icontext k-grid-delete" href="checkin_name_version_setting.php?edit=#=checkin_version_id#"><i class="icon-edit"></i> Edit</a> 
                                            <a class="k-button k-button-icontext k-grid-delete" onclick="javascript:deleteClick(#= checkin_version_id #);" ><span class="k-icon k-delete"></span> Delete</a>
                                        </script>
                                            ...
                                            function deleteClick(id) {
                                                ...
                                                if (c === true) {
                                                    $.ajax({
                                                        type: "POST",
                                                        dataType: "json",
                                                        url: "./controller/checkin_name_version_setting.php",
                                                        data: {id: id},
                                 ...
                                 <?php /*   <table class="table table-striped" id="data-table">
                                    ...
                                    <tbody>
										...
										if($input_status==1)
										{
											//$sql_checkin_version=$obj->SelectAll($table);
										...
										elseif($input_status==5)
										{
												...
												$sql_checkin_version=$obj_report_chain->SelectAllByID_Multiple_Or("checkin_version_store",$array_ch,"store_id","1");
										}
										else
										{
										$sql_checkin_version=$obj->SelectAllByID("checkin_version_store",array("store_id"=>$input_by));	
										}
										$i=1;
                                        ...
                                        <tr>
                                            <td><?php echo $i; ?></td>
                                            <td><?php echo $checkin_version->id; ?></td>
                                            <td><?php echo $checkin_version->name; ?></td>
                                            <td><?php echo $obj->SelectAllByVal($table2,"id",$checkin_version->checkin_id,"name"); ?></td>
                                            <?php if($input_status==1){ ?>
                                            ...
                                            <?php } ?>
                                            <td>
                                                <a href="<?php echo $obj->filename(); ?>?edit=<?php echo $checkin_version->id; ?>" class="btn btn-primary hovertip" title="Edit Detail"><i class="icon-edit"></i></a> 
                                                <?php if($input_status==1 || $input_status==5){ ?>
                                                <a href="<?php echo $obj->filename(); ?>?del=<?php echo $checkin_version->id; ?>" class="btn btn-danger hovertip" title="Delete"><i class="icon-remove"></i></a>
                                                <?php }else{ ?>
                                                <a href="<?php echo $obj->filename(); ?>?del=<?php echo $checkin_version->id; ?>&amp;sid=<?php echo $checkin_version->sid; ?>" class="btn btn-danger hovertip" title="Delete"><i class="icon-remove"></i></a>
            ...
            <script>
                nucleus("select[name='checkin']").kendoDropDownList({
                    optionLabel: " Please Select Checkin Type  "
