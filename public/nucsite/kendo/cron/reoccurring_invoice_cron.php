...
if (!empty($sql_product)) {
    foreach ($sql_product as $product):
        ...
        //echo NextDueDateEmailDate($duedate, $product->frequencyst)."<br>";
        if (NextDueDateEmailDate($duedate, $product->frequencyst)==date('Y-m-d')) {
           // echo NextDueDateEmailDate($duedate, $product->frequencyst);
            ...
            $EmailBody = file_get_contents($obj->baseUrl('cron/email/email/body/reccuring_bill_reminder.php?date=' . $duedate.'&bill='.$product->total_price));
            if ($sendMailStatus = sendEmailFunction($product->email,$product->customer_name,'support@nucleuspos.com', $EmailSubject, $EmailBody) == true) {
